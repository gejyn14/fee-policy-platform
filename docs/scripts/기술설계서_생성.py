...
r = m.add_run('버전 0.9  ·  2026-07-05  ·  시스템/아키텍처 기술 상세'); set_ko(r); r.font.size = Pt(10.5); r.font.color.rgb = GREY
doc.add_paragraph()
...
table(['용어', '정의'], [
    ...
    ['정책 우선순위 인덱스', '기본+전 이벤트의 정적 요율 랭킹(순위는 계좌 무관, 자격은 조회 시 필터). 룰 발효·만료 때만 재산정.'],
...
table(['테이블/구조', '주요 컬럼', '설명'], [
    ...
    ['정책 우선순위 인덱스', 'fee_key 매칭 → [정책, rank 오름차순]', '기본+전 이벤트의 정적 요율 랭킹. 룰 변경 때만 재산정(5장)'],
], widths=[3.6, 8, 4])
...
num([
    ...
    '이벤트(EVENT) :: 협의가 없을 때, 정책 우선순위 순위에서 자격 통과 최저가',
...
mono('후보 = 협의(활성·scope매칭) ∪ 이벤트(대상·기간 통과) ∪ 기본(BASE)\n'
     'if   협의 후보 있음:    승자 = argmin[협의] (고객부담, +동률규칙)   # 협의 무조건 우선\n'
     'elif 이벤트 후보 있음:  승자 = argmin[이벤트] (고객부담, +동률규칙)  # 이벤트는 지배관계로 기본 이하\n'
     'else:                승자 = 기본(BASE)\n'
     '\n'
     '# 동률규칙 — 고객부담이 같은 후보가 둘 이상일 때\n'
     '#            (서로 다른 이벤트가 같은 요율인 경우 포함):\n'
     '#   ① 더 구체적인 적용범위  →  ② 규칙 id(결정성)')
para('즉 “협의 → min(이벤트) → 기본” 순입니다. 각 계층 안에서는 사전 산정된 요율 순위(구조 그룹)가 '
     '가장 낮은 후보를 고르되, 서로 다른 이벤트의 요율이 같으면(동률) 위 동률규칙(①→②, 상세는 5.4)으로 '
     '단 하나를 확정합니다. 고객이 내는 금액은 어차피 같으므로, 이는 “어느 우대로 적용·집계할지”를 언제 조회해도 '
     '같은 결과로 못박기 위한 것입니다. 이벤트가 기본보다 반드시 싸다는 것은 등록 시 지배관계 검증으로 보장되므로, '
...
para('구현은 이를 하나의 정렬로 표현합니다: ① 협의 무조건 우선 → ② 사전 산정 요율 순위(구조 그룹) → ③ 동률 '
     'tie-break(구체성 → id). 이벤트가 기본보다 반드시 싸다는 것은 지배관계로 보장되므로, 순위에서 유효 이벤트는 '
     '항상 기본보다 위(더 낮음)에 오고 결과는 “협의 → min(이벤트) → 기본”과 동일합니다.')

h('5.2 정책 우선순위 — 사전 산정', level=2)
para('“언제 우선순위를 정하는가”의 답: 기본 + 전(全) 이벤트의 요율 순위를 체결 때가 아니라 룰이 발효·만료될 때 '
     '미리 산정합니다. 계좌 특정 이벤트(신청/가입/휴면복귀/지정타겟/상대형)도 순위에 포함하며, 계좌 여부는 순위가 '
     '아니라 조회 시 자격 필터로 가립니다.')
b('대상 :: 기본(BASE) + 활성 이벤트 전부(apply_mode 무관). 순위는 계좌와 무관 — 계좌 특정성은 자격 필터의 몫')
b('순위 기준 :: 구조 그룹별 요율 파라미터 직접 비교(정률=요율, 정액=정액). 합성 기준체결 없음. 같은 feeKey는 '
  '구조가 단일이라 같은 단위끼리만 비교되고, 등록 시 단조·비교차가 보장돼 순서가 가격과 무관')
b('winnerFor(feeKey, 계좌, 이력) :: 순위를 위에서 내려가며 자격(대상·기간) 통과 첫 정책 반환(재계산 없음)')
b('재산정 시점 :: 룰 발효/만료(활성 집합 변화) 시에만. 체결마다 다시 계산하지 않음')
para('체결 시 협의가 없으면 이 순위를 내려가며 자격 필터로 “이 계좌가 받는 첫 정책”을 확정합니다. 계좌 특정 이벤트 '
     '대상이 아닌 대다수 계좌는 그것들을 건너뛰고 기본/전체 이벤트에 착지합니다.')
...
table(['단계', '읽는 테이블 / 인덱스', '키', '반환'], [
    ...
    ['③ 순위 하강 + 자격 필터', '정책 우선순위 인덱스 (+ ENROLLMENT·타겟목록·기간)', 'feeKey → 랭킹, account_id → 자격', '이 계좌가 받는 첫 대상(이벤트/기본)'],
    ['④ 승자 확정', '(② 또는 ③)', '—', '협의 있으면 협의, 없으면 ③ 결과'],
    ['⑤ 요율표 평가', 'FEE_SCHEDULE·COMPONENT·RATE_BAND', 'schedule_id', '자사·유관기관·세금 금액'],
    ['⑥ 원장 반영', '(원장)', '—', '실제 부과·잔고 반영'],
], widths=[3, 6, 4, 3])
para('②에서 활성 협의가 있으면 그것이 승자입니다(협의 무조건 우선). 없으면 ③에서 미리 산정된 요율 순위를 위에서 '
     '내려가며 이 계좌의 자격(대상·기간)을 통과하는 첫 정책을 승자로 확정합니다(ENROLLMENT는 별도 단계가 아니라 '
     '하강 중 자격 필터). ⑤는 미리 저장하지 않고 요율표×체결로 즉석 계산합니다.')
pic('d9_swimlane', w=16.8)

h('6.2 순위 하강과 자격 게이트', level=2)
para('resolve는 후보를 모아 재계산하지 않고, 협의를 먼저 본 뒤 사전 산정 순위를 내려가며 자격을 겁니다.')
b('협의 :: NEGO_GRANT 중 account_id 일치 · status=활성 · valid_from~valid_to 내 · scope 매칭 → 있으면 무조건 승자')
b('순위 하강 :: 협의가 없으면 feeKey에 매칭되는 순위를 위에서부터, 각 정책에 자격 게이트 적용 → 첫 통과 정책')
b('자격 게이트 :: 기본은 항상 통과 / 이벤트는 대상 게이트(isTarget)·기간 게이트(isBenefitActive) 통과분만')
para('대상 게이트: 타겟추출형=전 계좌(또는 지정 계좌) / 신청·가입=ENROLLMENT 이력 존재 / 휴면복귀=복귀 계좌. '
     '기간 게이트: 캘린더형=규칙 기간 내 / 상대형=가입일~가입일+N(신청 마감과 무관).')

h('6.3 순위 확정과 캐시', level=2)
b('순위 :: ① 협의 무조건 우선 → ② 사전 산정 요율 순위(구조 그룹, 정률=요율/정액=정액) → ③ 동률 tie-break(구체성 → id)')
...
b('전체(지정 계좌 없음) :: 적용범위(scope)에 맞는 모든 계좌가 자동 대상. 체결 시 자격 게이트를 항상 통과하므로, '
  '순위에서 이 정책 이상으로 싼 계좌 특정 이벤트가 없으면 곧바로 승자가 됩니다')
b('지정 계좌(target_account_ids) :: CSV/Excel로 계좌 리스트를 업로드해 특정 타겟에만 주입. 순위 하강 중 대상 '
  '게이트가 계좌 소속을 확인 — 목록에 없으면 건너뜀')
para('두 경우 모두 정책 우선순위 순위에는 함께 들어갑니다(순위는 계좌 무관). 차이는 조회 시 자격 필터뿐 — 전체는 '
     '항상 통과, 지정은 업로드 목록 소속일 때만 통과합니다. 즉 타겟추출형은 “계좌 행위 없이 시스템이 대상을 정하는” 경로입니다.')
...
out = '/Users/yujin-an/dev/fees/docs/수수료정책플랫폼_기술설계서_v0.9.docx'
