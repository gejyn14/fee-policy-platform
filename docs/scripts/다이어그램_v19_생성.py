# -*- coding: utf-8 -*-
"""기술설계서 v1.9용 프로세스 도식 1종 (PIL, 한글, 2x 해상도).

한국식 컨설팅 프로세스 도식: 새 수수료 정책의 여정을 좌→우 5단계로 펼치고,
1차전(승인 때 사전 랭킹)에서 일어난 두 변화를 강조한다.
  ① 색인 키 4축 → 8축 완전 키화(+세션·채널·유동성(메이커/테이커)·매수/매도)
  ② 1차전을 인메모리 배치 계산에서 물리 테이블(순위값 스탬프 + 후보 색인)로 이동

기존 다이어그램(다이어그램_v18_생성.py)과 같은 팔레트·폰트를 쓴다.
실행: python3 docs/scripts/다이어그램_v19_생성.py  (레포 루트 기준)
산출: docs/diagrams/v19_정책여정_프로세스.png
"""
import os
import math
...
def chiprow(d, x, y, items, h=30):
    """items: (text, fill, outline, tcolor, bold). 자동 폭. 반환: 다음 x."""
    cx = x
    for text, fill, outline, tcolor, bold in items:
        f = FB(12) if bold else F(12)
        w = d.textlength(text, font=f) / S + 26
        chip(d, cx, y, w, h, text, fill, outline, tcolor, bold)
        cx += w + 8
    return cx
...
def phase(d, x, y, w, h, num, name, timing, timing_c, lines,
    ...
    tf = F(11)
    tw = d.textlength(timing, font=tf)
    tx = x + (w - (tw / S + 16)) / 2
    rrect(d, tx, y + 40, tw / S + 16, 20, WHITE, timing_c, width=2, radius=10)
    d.text((tx * S + 8 * S, (y + 40) * S + 3 * S), timing, font=tf, fill=timing_c)
    yy = y + 70
    bf = F(12)
    for ln in lines:
        for k, sub in enumerate(wrap(d, ln, bf, (w - 26) * S)):
            d.text(((x + 15) * S, yy * S), ('·  ' if k == 0 else '    ') + sub, font=bf, fill=GREY)
            yy += 12 * 1.55
        yy += 3
...
def main():
    ...
    phases = [
        ...
        (bx0 + 2 * (bw + gap), '3', '1차전: 사전 랭킹', '승인 때 1회', GREEN_T, GREEN_F, GREEN_O, GREEN_T,
         ['계좌 무관 스코프 키 경쟁', '순위값 물리 저장', '8축 조합별 후보 색인']),
    ...
    for i in range(n - 1):
        x1 = bx0 + i * (bw + gap) + bw
        arrow(d, (x1 + 3, by + bh / 2), (x1 + gap - 2, by + bh / 2), color=NAVY, w=4)
    ...
    # 카드 A: 4축 → 8축
    ax, ay, aw, ah = 76, sy + 34, 616, 244
    rrect(d, ax, ay, aw, ah, WHITE, GREEN_O, width=2, radius=14)
    ltext(d, ax + 20, ay + 16, '변화 ①  색인 키: 4축 → 8축 완전 키화', FB(15), GREEN_T)
    ltext(d, ax + 20, ay + 46,
          '수수료 승자를 가르는 계좌 무관 축을 남김없이 색인 키로 승격', F(12), GREY)

    ltext(d, ax + 20, ay + 76, '기존 4축', FB(12), NAVY)
    base = [(c, BLUE_F, BLUE_O, NAVY, False) for c in ['자산군', '조회구분', '거래소', '품목']]
    chiprow(d, ax + 20, ay + 94, base)

    ltext(d, ax + 20, ay + 132, '+ 신규 4축', FB(12), AMBER_T)
    new4 = [(c, AMBER_F, AMBER_O, AMBER_T, True)
            for c in ['+ 세션', '+ 채널', '+ 유동성(메이커/테이커)', '+ 매수/매도']]
    chiprow(d, ax + 20, ay + 150, new4)

    ltext(d, ax + 20, ay + 190,
          '왜 승격? 세션·채널·유동성·매매구분만 한정한 이벤트가 승자를 바꿈', F(12), GREY)
    ltext(d, ax + 20, ay + 212,
          '(야간·MTS·메이커 우대·매도 면제). 8축을 키에 넣어야 2차전이 순수 키 조회 + 게이트로 끝남.',
          F(12), GREY)

    # 카드 B: 인메모리 → 물리 테이블
    bx, byy, bwid, bhg = ax + aw + 20, sy + 34, W - 48 - 28 - (ax + aw + 20), 244
    rrect(d, bx, byy, bwid, bhg, WHITE, GREEN_O, width=2, radius=14)
    ltext(d, bx + 20, byy + 16, '변화 ②  인메모리 배치 계산 → 물리 테이블', FB(15), GREEN_T)

    ltext(d, bx + 20, byy + 56, '기존', FB(12), GREY)
    rrect(d, bx + 20, byy + 76, bwid - 40, 46, GREY_F, GREY_O, width=2, radius=10)
    ltext(d, bx + 34, byy + 87, '배치 실행 중 후보·순위 인메모리 재계산', F(12), GREY)
    ltext(d, bx + 34, byy + 104, '(계좌×셀마다 다시 훑음)', F(11), GREY)

    arrow(d, (bx + bwid / 2, byy + 128), (bx + bwid / 2, byy + 146), color=GREEN_T, w=3)

    ltext(d, bx + 20, byy + 152, '현재 · 물리 테이블 2종', FB(12), GREEN_T)
    rrect(d, bx + 20, byy + 172, bwid - 40, 46, GREEN_F, GREEN_O, width=2, radius=10)
    ltext(d, bx + 34, byy + 183, '순위값 스탬프 · 8축 조합별 후보 색인', FB(12), GREEN_T)
    ltext(d, bx + 34, byy + 200, '(tie_order · specificity 포함)', F(11), GREEN_T)

    # ── 하단 밴드
    fy = ay + ah + 22
    rrect(d, 76, fy, W - 152, 104, WHITE, GREEN_O, width=2, radius=14)
    ltext(d, 96, fy + 15, '한 번 물리화하면 — 이후 모든 경로는 저장된 순위값을 읽기만 한다', FB(15), GREEN_T)
    readers = [(r, BLUE_F, BLUE_O, NAVY, False)
               for r in ['일배치 전체 재산출', '수시 증분', '화면 조회', '미스 경로(기본 해석)']]
    chiprow(d, 96, fy + 44, readers)
    ltext(d, 96, fy + 80,
          '효과: 우선순위 판단은 승인 시점 한 곳에만 존재 · 배치는 결과 정렬·적재로 수렴(2억 행 6분 실측 연장선)',
          F(12), GREY)

    path = f'{OUT}/v19_정책여정_프로세스.png'
    img.save(path)
