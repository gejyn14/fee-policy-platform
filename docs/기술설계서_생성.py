...
h('5.1.1 우선순위 계산 (일반)', level=3)
para('승자 결정은 “계층 우선 + 계층 내 최저가”로 정의됩니다. 의사코드로 표현하면 다음과 같습니다.')
mono('후보 = 협의(활성·scope매칭) ∪ 이벤트(대상·기간 통과) ∪ 기본(BASE)\n'
     'if   협의 후보 있음:    승자 = argmin[협의] (고객부담)     # 협의 무조건 우선\n'
     'elif 이벤트 후보 있음:  승자 = argmin[이벤트] (고객부담)   # 이벤트는 지배관계로 기본 이하\n'
     'else:                승자 = 기본(BASE)')
para('즉 “협의 → min(이벤트) → 기본” 순입니다. 각 계층 안에서는 고객부담(공동 probe grid의 평균 고객부과액)이 '
     '가장 낮은 후보를 고릅니다. 이벤트가 기본보다 반드시 싸다는 것은 등록 시 지배관계 검증으로 보장되므로, '
     '“이벤트 계층의 최소가 곧 기본을 이긴다”가 성립합니다(4.3·5.3·11장).')
table(['계층', '선택 규칙', '비고'], [
    ['협의(NEGOTIATED)', '활성 grant가 있으면 그 계층에서 argmin(고객부담)', '계층 자체가 이벤트·기본보다 앞섬(무조건 우선)'],
    ['이벤트(EVENT)', '대상·기간 통과 이벤트들 중 argmin(고객부담)', '지배관계로 기본 이하 → min(이벤트)가 승자'],
    ['기본(BASE)', '협의·이벤트가 없을 때 fallback', '항상 후보에 포함되는 기준선'],
], widths=[3.5, 8, 4])
para('구현은 이 계층 우선을 하나의 정렬로 표현합니다: ① 협의 우선(negoFirst) → ② 고객부담 최저(min) → ③ 계층'
     '(이벤트>기본) → ④ 이하 동률 tie-break(5.4). ②에서 이벤트·기본을 함께 최저가로 비교하지만, 지배관계로 '
     '유효 이벤트는 항상 기본 이하이므로 결과는 위 “협의 → min(이벤트) → 기본”과 동일합니다.')
