...
## 3.1 개정 (v0.3.1) — 등급을 수수료 결정 경로에서 제거

핵심 재확인: 이 플랫폼의 목표는 **`계좌→등급→(등급,품목)→요율표` 직조회 구조를 벗어나는 것**. 따라서 등급을
수수료 결정의 pivot으로 쓰지 않는다.

- **등급은 수수료 플랫폼이 읽지 않는다.** 계좌 마스터 속성으로 존재는 하되(서비스·PB용) `isTarget`/바인딩
  결정에 관여하지 않음. 화면(①·계좌 드롭다운)에서 등급 표시 제거.
- **대상 판정 모델**: 각 룰이 스스로 대상을 선언(전체 / 지정 계좌목록 / 조건(지표 임계값) / 신청 / 휴면복귀).
  플랫폼은 이 계좌×품목에 걸리는 룰을 모아 경쟁. 등급 pivot 없음. 6개월 지표는 **룰의 자격 조건**으로서는
  유효(예: 협수 평균자산 ≥ 5억) — 이는 등급표 직조회와 달리 "룰이 자기 대상을 정의"하는 정상 동작.
- **① 재구성**: "계좌 확인(등급/지표)" → **"대상 판정"**. 실제 `isTarget`이 읽는 것만 노출 —
  계좌번호·이름·휴면복귀 여부·협의수수료 신청 내역. 내레이션도 "등급표 직조회를 하지 않고 각 룰의 대상 선언에
  이 계좌가 걸리는지 판정"으로 교체.
- 지표를 결정에 실제 연결(`evalCondition`을 `isTarget`에 물림)하는 것은 v0.4(등급/지표 재산정 → 협수 조건
  평가 배치)의 범위.
