...
>
> **구현 중 조정(2026-07-07)**:
> - **배정판 = 우대(EVENT/NEGOTIATED) 승자만 저장.** 기본(BASE) 승자는 행을 쓰지 않는다. 원장 조회가 0건이면 그것이 정상 경로이며 기본수수료를 직접 해석해 적용한다(자산군·조회구분으로 BASE 룰 조회 — 전 계좌 공통·결정적). 근거: 기본은 계좌마다 다르지 않아 계좌×셀로 복제 저장하면 억 단위 낭비. 배정판이 "기본 대비 우대분"만 담으면 수백만 규모로 축소되고 배치도 그만큼 가벼워진다. → §5.3·§6.2·§6 배치·§10 시나리오에 반영.
> - 스택: 로컬 환경에 Java 21·Docker 부재 → **Java 17 + Podman**(로컬 Postgres 16, 호스트 5433)로 구현. `docker-compose.yml`은 팀 공유용으로 유지. 도메인 코드는 Java 17 호환(record·text block·switch).
...
- `account`(계좌: 등급·휴면복귀·6개월 지표), `account_product_group`(계좌가 개설한 상품군 — 셀 유니버스 pruning 기준), `product`(품목 마스터: 자산군·거래소·품목·세션 배열), `qualify_policy`(협의 자격 기준)
