...
describe('ResolveCache', () => {
  ...
  it('set 후 get 적중(hits++), 미설정 get miss(misses++)', () => {
    ...
    c.set('A', key(), { scheduleId: 'S1', sourceRuleId: null });
    expect(c.get('A', key())).toEqual({ scheduleId: 'S1', sourceRuleId: null }); // hit
    const s = c.stat();
  ...
  it('계좌·feeKey가 다르면 별도 항목', () => {
    c.set('A', key(), { scheduleId: 'S1', sourceRuleId: null });
    c.set('B', key(), { scheduleId: 'S2', sourceRuleId: null });
    c.set('A', key({ channel: 'HTS' }), { scheduleId: 'S3', sourceRuleId: null });
    expect(c.stat().size).toBe(3);
  });

  it('invalidateAccount는 그 계좌 항목만 제거하고 건수 반환', () => {
    c.set('A', key(), { scheduleId: 'S1', sourceRuleId: null });
    c.set('A', key({ channel: 'HTS' }), { scheduleId: 'S2', sourceRuleId: null });
    c.set('B', key(), { scheduleId: 'S3', sourceRuleId: null });
  ...
  it('invalidateByScope(pred)는 조건 매칭 키만 제거', () => {
    c.set('A', key({ exchange: 'KRX' }), { scheduleId: 'S1', sourceRuleId: null });
    c.set('A', key({ exchange: 'NXT' }), { scheduleId: 'S2', sourceRuleId: null });
